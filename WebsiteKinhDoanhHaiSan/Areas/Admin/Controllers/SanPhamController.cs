    ...
    public class SanPhamController : BaseController
    {
        ...
        public JsonResult DsSanPham(int trang = 1)
        {
            try
            {
                ...
                var dsSanPham = (from k in db.SanPhams where k.DaXoa!=1 select new { k.MaSP, k.TenSanPham, k.HinhAnh, k.DVT, k.SoLuong, k.DonGiaNhap, k.DonGiaBan, k.NhaCungCap , k.Loai }).ToList();
        ...
        //////
        [HttpPost]
        public JsonResult CreateUpdate(string MaSp,string tenSP,HttpPostedFileBase HinhAnh,string DVT,string SoLuongSP,string DonGiaNhap,string DonGiaBan,string idNCC,string idLoai,string MoTa)
        {
            try
            {
                db.Configuration.ProxyCreationEnabled = false; // cau hinh proxy cho database 
                ...
                if (MaSp == "") // truong hop them moi
                {
                    ...
                    sp.DonGiaNhap = double.Parse(DonGiaNhap);
                    sp.DonGiaBan = double.Parse(DonGiaBan);
                    sp.MaNCC = int.Parse(idNCC);
        ...
        public JsonResult Delete(int id)
        {
            try
            {
               ...
               // }
                sp.DaXoa = 1;
                db.SaveChanges();
                return Json(new { trangthai = 0, msg = "Xóa San Pham có tên : " + sp.TenSanPham + " thành công!" }, JsonRequestBehavior.AllowGet);
